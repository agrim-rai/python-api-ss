...
# --- Suspicion Patterns ---
# (Keep all your patterns and weights as they are)
AI_DOMAINS = {
    "openai.com", "chatgpt.com", "claude.ai", "anthropic.com",
    "gemini.google.com", "bard.google.com", "perplexity.ai", "blackbox.ai",
    "phind.com",
}
SOLUTION_DOMAINS = {
    "stackoverflow.com", "github.com", "geeksforgeeks.org", "leetcode.com",
    "medium.com", "dev.to", "tutorialspoint.com", "w3schools.com",
    "programiz.com", "chegg.com", "coursehero.com",
}
SEARCH_DOMAINS = {
...
SUSPICIOUS_KEYWORDS = [
    "solution", "answer", "code", "solve", "cheat", "hack", "discussion",
    "discuss", "forum", "community", "tutorial", "guide", "example",
    "reference", "pastebin", "jsfiddle", "codepen", "gpt", "claude",
    "gemini", "bard", "ai", "llm", "translate",
]
LEGITIMATE_PLATFORM_KEYWORDS = [
    "problems", "problemset", "list", "submissions", "contest", "profile",
    "explore", "ranking", "editorial",
]
SCORE_WEIGHTS = {
    "TO_AI": 10, "TO_SOLUTION_DOMAIN_WITH_KEYWORDS": 8,
    "TO_SOLUTION_DOMAIN_GENERIC": 5, "TO_GITHUB_REPO": 6,
    "TO_SEARCH_ENGINE": 4, "TO_EXTERNAL_APPLICATION": 5,
    "TO_SUSPICIOUS_KEYWORD_ONLY": 3, "FROM_AI": 1, "FROM_SOLUTION": 1,
...
# --- Helper Functions ---
# (Keep get_domain, contains_keywords, normalize_problem_identifier as they are)
def get_domain(url):
    """Extracts the domain name (e.g., 'google.com') from a URL."""
    if not url or not isinstance(url, str) or not url.startswith(('http://', 'https://')):
        return None
    try:
        ...
        logging.warning(f"Could not parse URL: {url}") # Log goes to stderr
        return None
...
# --- Core Analysis Logic ---
# (Keep analyze_tab_switch function exactly as it is)
def analyze_tab_switch(doc):
    """
    Analyzes a single tab switch document for suspicious activity.
    Args: doc (dict): The document fetched from MongoDB.
    Returns: dict: An analysis result.
    """
    suspicion_score = 0
    reasons = []
    doc_id = str(doc.get("_id", {}))
    ...
    if isinstance(timestamp_val, datetime):
        try:
            ...
            logging.warning(f"Could not process timestamp datetime object {timestamp_val}: {e}") # Log goes to stderr
            timestamp_ms = None
            timestamp_iso = "N/A"
    elif timestamp_val is not None:
        logging.warning(f"Timestamp field is not a datetime object: type={type(timestamp_val)}, value={timestamp_val}") # Log goes to stderr
        if isinstance(timestamp_val, dict):
             try:
                 ts_long = timestamp_val.get("$date", {}).get("$numberLong")
                 if ts_long:
                  ...
                  logging.warning(f"Could not parse dictionary timestamp {timestamp_val}: {e}") # Log goes to stderr
    ...
    to_title = doc.get("toTitle", "")
    to_domain = get_domain(to_url)
    to_text = f"{to_url} {to_title}".lower()
    ...
    if to_url == "external_application":
        ...
        if to_domain in AI_DOMAINS:
            ...
            found_kw, matched_kw = contains_keywords(to_text, SUSPICIOUS_KEYWORDS)
            is_github_repo = to_domain == "github.com" and len(urlparse(to_url).path.split('/')) > 2
            is_leetcode_discuss = to_domain == "leetcode.com" and "/discuss/" in to_url.lower()
            is_different_problem = False
            if to_domain == f"{platform}.com":
                 current_problem_norm = normalize_problem_identifier(problem_id) or normalize_problem_identifier(problem_title)
                 to_problem_norm_from_url = normalize_problem_identifier(to_url)
                 to_problem_norm_from_title = normalize_problem_identifier(to_title)
                 if current_problem_norm and \
                    ((to_problem_norm_from_url and to_problem_norm_from_url != current_problem_norm) or \
                     ...
                     if not found_legit_kw:
                         ...
                         suspicion_score += SCORE_WEIGHTS["WITHIN_PLATFORM_TO_DIFFERENT_PROBLEM"]
                         reasons.append(f"Switched TO different problem page/URL on {platform}: {to_title or to_url}")
                 if is_leetcode_discuss and not is_different_problem:
                     suspicion_score += SCORE_WEIGHTS["WITHIN_PLATFORM_TO_DISCUSSION"]
                     reasons.append(f"Switched TO {platform} discussion forum: {to_title or to_url}")
            if is_leetcode_discuss or is_different_problem:
                pass
            elif is_github_repo:
                suspicion_score += SCORE_WEIGHTS["TO_GITHUB_REPO"]
                reasons.append(f"Switched TO GitHub repository: {to_url}")
    ...
    return {
        "document_id": doc_id, "username": username, "problem_id": problem_id,
        "platform": platform, "timestamp": timestamp_iso,
        "suspicion_score": final_score, "reasons": reasons,
        "details": {
            "from": {"url": from_url, "title": from_title},
            "to": {"url": to_url, "title": to_title}
        }
...
def fetch_document_by_id(document_id):
    ...
    try:
        ...
        try:
            ...
            # Log error to stderr, return error message for JSON output
            logging.error(f"Invalid ObjectId format: {document_id} - {e}")
            return None, f"Invalid ObjectId format: {document_id}"

        logging.info(f"Attempting to fetch document with _id: {obj_id}") # Log goes to stderr
        document = collection.find_one({"_id": obj_id})

        if document:
            logging.info(f"Document found.") # Log goes to stderr
            return document, None
        else:
            logging.warning(f"No document found with _id: {document_id}") # Log goes to stderr
            return None, f"No document found with _id: {document_id}"

    except Exception as e:
        logging.error(f"Database connection or query failed: {e}", exc_info=True) # Log exception traceback to stderr
        return None, f"Database error: {e}"
    finally:
        if client:
            client.close()
...
if __name__ == "__main__":
    ...
    # This case should technically not be reached if error is None and doc_content is None,
    # but included for completeness.
    if not doc_content:
         print(json.dumps({"error": f"Failed to retrieve document {document_id} but no specific error returned."}, indent=2))
         sys.exit(1)

    # Proceed only if document is found and is of the correct type
    if doc_content.get("eventType") != "tab_switch":
         # Print error JSON to stdout
         print(json.dumps({"error": f"Document {document_id} is not a 'tab_switch' event (eventType: {doc_content.get('eventType')})"}, indent=2))
         sys.exit(1)

    # --- Add try/except around the analysis ---
    try:
        analysis_result = analyze_tab_switch(doc_content)
        # Print the final analysis result JSON to stdout
        print(json.dumps(analysis_result, indent=2))
    except Exception as analysis_error:
        # Log the detailed error to stderr
        logging.error(f"Error during analysis of document {document_id}: {analysis_error}", exc_info=True)
        # Print a user-friendly error JSON to stdout
        print(json.dumps({
            "error": f"Analysis failed for document {document_id}",
