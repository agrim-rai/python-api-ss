...
#         # Print ONLY the JSON result to stdout, with no logging mixed in
...
# Configure logging to stderr explicitly
logging.basicConfig(
    level=logging.INFO, 
    format='%(asctime)s - %(levelname)s - %(message)s',
    stream=sys.stderr  # This is the key change - explicitly log to stderr
)
...
if __name__ == "__main__":
    # Check arguments
    if len(sys.argv) < 2:
        # Print to stdout because this is what FastAPI will capture as the result
    ...
    if doc_content:
        if doc_content.get("eventType") != "tab_switch":
            print(json.dumps({"error": f"Document {document_id} is not a 'tab_switch' event"}))
            sys.exit(1)

        # Perform analysis and return the result
        analysis_result = analyze_tab_switch(doc_content)
        
        # Print ONLY the JSON result to stdout, with no indentation to minimize size
        print(json.dumps(analysis_result, default=str))
        sys.exit(0)
    else:
        print(json.dumps({"error": f"Failed to retrieve document {document_id}"}))
        sys.exit(1)